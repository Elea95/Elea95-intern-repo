...
```python
def compute_product(num1: int, num2: int, values: list) -> int:
...
def compute_sum(num1: int, num2: int) -> int:
    return num1 + num2

def calculate_product_or_sum(num1: int, num2: int, values: list):
    """
    Decides action based on input:
    - If num1 > 10 and num2 < 5: returns product.
    - If num2 > 10: prints list.
    - Otherwise: returns sum.
    """
    if num1 > 10 and num2 < 5:
        return compute_product(num1, num2, values)

    if num2 > 10:
        print_list(values)
        return  # Explicitly returns None if printing is the only task

    return compute_sum(num1, num2)
```
...
function calculateDiscount(price, discount) {
  ...
  if (price < 0 || discount < 0 || discount > 1) {
    throw new Error(
      "Invalid values: price must be positive, discount must be between 0 and 1."
    );
  }
